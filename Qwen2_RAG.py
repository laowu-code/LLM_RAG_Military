...
def stream_response(prompt, rag_chain):
    ...
    response_text=rag_chain.invoke( {"input": prompt},
    config={"configurable": {"session_id": "abc123"}},)["answer"]

    for token in response_text:
        response_stream += token
        response_container.write(response_stream)
        time.sleep(0.02)  # 模拟流式输出的延迟
    # response_container.write(response_rag["answer"])
...
# )
    ...
    def get_rag():
        ...
        history_aware_retriever = create_history_aware_retriever(
            llm, retriever_, contextualize_q_prompt)
...
# )
    ...
    if st.sidebar.button("清除所有聊天记录"):
        st.session_state["messages"] = [{"role": "assistant", "content": "聊天记录已清除。有什么可以帮您的？"}]
        chat_history.clear()
